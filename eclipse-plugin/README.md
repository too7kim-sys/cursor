...
| **오류 수정(Problems 퀵픽스)** | ✅ | 오류 줄에 커서 → `Ctrl+Alt+.` → 해당 컴파일 오류/경고를 모델이 수정 → diff 확인 후 제자리 적용 |
| **채팅→에디터 적용** | ✅ | 채팅 답변의 마지막 코드블록을 뷰 툴바 **[에디터적용]** → 선택 영역 교체/커서 삽입(diff 확인) |
| **@파일/@선택 멘션** | ✅ | 입력창에 `@경로/파일.java`, `@selection` → 해당 파일/현재 선택 코드를 컨텍스트로 자동 첨부 |
| **대화 세션 저장/복원** | ✅ | 대화 내용·멀티턴 히스토리를 자동 저장해 재시작 후 복원, 툴바 **[새세션]**으로 초기화 |
