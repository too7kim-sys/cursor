...
| **고스트 완성(Copilot 식)** | ✅ | 커서에서 `Ctrl+Alt+Space` → 제안이 **회색 미리보기**로 표시 → `Tab` 수락 / `Esc`·다른 키 취소. **줄 끝에서만** 표시(오버레이 안정화) |
...
| **멀티파일 일괄 적용 패널** | ✅ | 툴바 **[변경적용]** → 답변의 **파일 지정 코드블록들**(```lang path``` / `파일: 경로` / `**경로**`)을 모아 체크 목록·diff로 검토 후 일괄 저장 |
...
| **변경 롤백/되돌리기(다단계)** | ✅ | 에디터 적용 변경은 **`Ctrl+Z`**, Agent 파일 변경은 툴바 **[되돌리기]** → **체크포인트(실행 단위) 히스토리**에서 시점 선택해 복구. 히스토리는 **프로젝트별 디스크 영속화**(재시작 후에도 유지). **새로 생성/이동된 파일은 되돌릴 때 삭제**되어 원래 상태로 정확히 복원. 모든 변경은 적용 전 미리보기/확인 |
...
- 테스트 본문: [`tests/TestRunner.java`](tests/TestRunner.java) (현재 194개 단언)
